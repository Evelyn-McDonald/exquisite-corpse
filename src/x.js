...
var reveal = $('#revealWrapper');

$(window).on("resize", setHeight).resize();

function setHeight() {
    var width = Math.max(build.width(), reveal.width());
...
function drawShadows() {
    ctx2.save();
    ctx2.beginPath();
    ctx2.arc(shadowCanvas.width/2, shadowCanvas.height/2 - 40, 340, 0, Math.PI * 2, false);
    ctx2.closePath();

    ctx2.clip();
        ctx2.fillStyle = 'white';
        ctx2.beginPath();
        ctx2.arc(shadowCanvas.width/2, shadowCanvas.height/2 - 38, 336, 0, Math.PI * 2, false);
        ctx2.closePath();
        ctx2.fill();
